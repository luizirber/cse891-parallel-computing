...
int distribute_vector(int vect[], int procs) {
    ...
    for (int i = 1; i < procs; ++i ) {
      chunk_size = calc_chunk_for_proc(i, N, procs);
      MPI_Send(vect + i * (N / procs), chunk_size, MPI_INT, i, DISTRIBUTE_TAG, MPI_COMM_WORLD);
    }
    return 0;
}
...
int receive_vector(int vect[], int procs, int rank) {
    MPI_Status status;
    int chunk_size = calc_chunk_for_proc(rank, N, procs);
...
int main(int argc, char **argv)
{
   ...
   chunk_size = calc_chunk_for_proc(rank, N, procs);
